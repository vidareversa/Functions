...
class Funciones {
  ...
  /*
   * para usar en vez de mysql_query (para extraer datos)
  */
  public function  mysqli_to_array($conexion, $sql, $primerRegistro = false) {
      $res = mysqli_query($conexion, $sql);
      $rows = array();
      if (is_object($res)) {
              if ($primerRegistro) { //segundo parametro, true para que solo devuelva un registro                   
  ...
  */
  function in_asociative_array ($array, $key, $key_value) {
      $within_array = false;
...
$trucomands[] = array(array('msgtext' => 'pepe', 'cantidad' => '2'),
...
if ($f->in_asociative_array($trucomands, 'msgtext', 'pepe')) {
  die("ESTA EN EL ARRAY");
} else {
  die("NO ESTA EN EL ARRAY");
}
